...
def parse_for(parser):
    """
    >>> import os
    >>> path = os.path.join(os.getcwd(), "tests", "pass", "sub", "for")
    >>> files = os.listdir(path)
    >>> for file in files:
    ...     parse_for(Parser(open(os.path.join(path, file), "r").read()))
    ast.For
    """
    parser.accept("for")
    try:
...
def parse_import(parser):
    """
    >>> toks = [x for x in tokenise("import x")]
    >>> parse_import(Parser(toks))
    ast.Import
    """
...
def test_string(function, string):
    p = Parser(string)
    return function(p)
